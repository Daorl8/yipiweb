...
## 2026-08-06 (6) — 스타일 태그를 이미지 오버레이로 이동
- (5)에서 `.gstyle`를 `.gmeta`(flex: gname·gcat 양끝)에 3번째로 넣었더니 하단 텍스트 과다+카드별 높이 어긋남(가린 타투 2줄 등, 다올 지적).
- 해결: 마크업 그대로 두고 **CSS만** — `.wcard{position:relative}` + `.gmeta .gstyle`를 이미지 우상단 절대배치 오버레이(반투명 다크 칩·흰 글씨·blur·pointer-events:none)로 전환. flow에서 빠져 하단은 이름+업종만 남음 → 정렬 회복·번잡 해소.
- index.html CSS 2줄만 변경.

## 2026-08-06 (5) — 샘플 카드 디자인 스타일 태그 (ONBUILD 항목2)
- #work 17종 카드 `.gmeta`에 `.gstyle` 한 줄 추가(예: 가린타투="다크 버밀리온·에디토리얼 볼드", 여백="스위스 미니멀·극단 여백"). 샘플을 "업종 사이트"가 아니라 **"디자인 역량 데모"로 리프레임** = 신뢰 자산(콘텐츠·사이트의 새 역할) 강화 + [[sample-palette-font-homogeneity]] "다 비슷" 체감에 차별 서사.
- 17개 태그는 다올 승인(2026-08-06). CSS `.gstyle`(작은 뮤트 라인) + 카드 17줄 편집. index.html만.
- ⚠️ **업종 카테고리 그룹핑(ONBUILD 항목3)은 보류** — #work가 마퀴(가로 무한스크롤)라 그룹/필터가 큰 구조변경. 샘플 대거 개편(#3) 논의와 묶어서 진행 예정(업종은 .gcat에 라벨로는 이미 존재).
...
- 안전장치: **실고객은 #work가 아니라 #case에 있어 자동 제외** + 향후 대비 `data-no-demo` opt-out(이 속성 붙은 wcard는 새 탭 유지).
- 마퀴 복제(17×2=34) 카드도 전부 자동 대상. 검증: 전 카드 URL/이름/업종 추출 정확·DV JS `node -c` 통과·무결성(931줄·</html>).
- ⚠️ 미배포분은 index.html 재커밋 필요(→CF). 프레임 내부 브릿지 배지 중복 억제·열람 비콘은 여전히 후속 후보.
