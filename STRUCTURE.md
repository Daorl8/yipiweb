...
## 페이지 (배포됨)
- `index.html` — 메인. 히어로·차별점(#value)·샘플 갤러리(#work, 17종)·실제 사례(#case, 썸머힐·네일어반)·진행방법(#process)·가격(#price)·문의(#contact, 폼 #cform). 푸터에 낙서장 링크. **인페이지 데모뷰어(#demoViewer, 2026-08-06 파일럿)**: `a.wcard[data-demo]` 클릭 → 모달 iframe으로 라이브 샘플 렌더 + 데스크톱/모바일 토글·새탭·닫기(ESC/백드롭). **현재 네일 아뜰리에(#10)만 연결**, 나머지는 새 탭 유지. 미배포(로컬만, GitHub 커밋은 판단 후). ⚠️확대 시 실고객 iframe 금지.
